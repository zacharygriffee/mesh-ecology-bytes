...
export const PARTICIPATION_DISTRIBUTABLE_PUBLIC_SEEDER_PROOF_SCHEMA =
  'mesh-ecology-bytes/participation-distributable-public-seeder-proof@1'
export const PARTICIPATION_DISTRIBUTABLE_PUBLIC_SEEDER_READBACK_SCHEMA =
  'mesh-ecology-bytes/participation-distributable-public-seeder-readback@1'
export const PARTICIPATION_DISTRIBUTABLE_PUBLIC_SEEDER_DOWN_SCHEMA =
  'mesh-ecology-bytes/participation-distributable-public-seeder-down@1'
...
function validateInputs({ distributable, storage, publicHyperdht }) {
  const issues = []
...
export function buildParticipationDistributablePublicSeederProof({
  distributable,
  ...
  const inputIssues = validateInputs({ distributable, storage, publicHyperdht })
  ...
  const status = inputIssues.length > 0
    ? 'public_seeder_blocked'
    : complete
      ? 'public_seeder_live'
      : 'public_seeder_unresolved'
  ...
  const proof = {
    schema: PARTICIPATION_DISTRIBUTABLE_PUBLIC_SEEDER_PROOF_SCHEMA,
    artifactKind: 'bytes_participation_distributable_public_seeder_proof',
    createdAt,
    status,
    ...
    storageRefs: {
      publisherStorageRef: storage ? `bytes-public-seeder-publisher:${createHash('sha256').update(path.resolve(storage)).digest('hex').slice(0, 16)}` : null,
    ...
    nextPosture: complete
      ? 'platform_or_conduit_materializes_while_bytes_public_seeder_is_live'
      : 'repair_bytes_public_seeder_availability'
  }
  proof.proofHash = sha256Json(withoutHash(proof, ['proofHash', 'proofRef']))
  proof.proofRef = `bytes-participation-distributable-public-seeder:${proof.proofHash.slice('sha256:'.length, 'sha256:'.length + 16)}`
  return proof
}

export function buildParticipationDistributablePublicSeederReadback({
  proof,
  proofPath = null,
  ...
  const readback = {
    schema: PARTICIPATION_DISTRIBUTABLE_PUBLIC_SEEDER_READBACK_SCHEMA,
    artifactKind: 'bytes_participation_distributable_public_seeder_readback',
    readAt,
    proofPath,
  ...
  readback.readbackRef = `bytes-participation-distributable-public-seeder-readback:${readback.readbackHash.slice('sha256:'.length, 'sha256:'.length + 16)}`
  return readback
}

export function buildParticipationDistributablePublicSeederDown({
  proof,
  ...
  const down = {
    schema: PARTICIPATION_DISTRIBUTABLE_PUBLIC_SEEDER_DOWN_SCHEMA,
    artifactKind: 'bytes_participation_distributable_public_seeder_down',
    stoppedAt,
    status: stopped ? 'public_seeder_stopped' : 'public_seeder_stop_unresolved',
  ...
  down.downRef = `bytes-participation-distributable-public-seeder-down:${down.downHash.slice('sha256:'.length, 'sha256:'.length + 16)}`
  return down
}

export function validateParticipationDistributablePublicSeederProof(proof = {}) {
  const issues = []
  if (proof.schema !== PARTICIPATION_DISTRIBUTABLE_PUBLIC_SEEDER_PROOF_SCHEMA) issues.push('schema_mismatch')
  if (!['public_seeder_live', 'public_seeder_unresolved', 'public_seeder_blocked'].includes(proof.status)) {
    issues.push('status_invalid')
  }
  if (proof.status === 'public_seeder_live') {
...
async function runUp(argv) {
  const distributablePath = argValue(argv, '--distributable')
  ...
  if (!distributablePath || !storage || !output || !readbackOutput || !pidFile) {
    console.error('Usage: node examples/participation-distributable-public-seeder-lifecycle.js up --distributable <path> --storage <path> --output <proof> --readback-output <readback> --pid-file <pid> --public-hyperdht')
    return 2
  }
  ...
  try {
    ...
    proof = buildParticipationDistributablePublicSeederProof({
      distributable,
      distributablePath,
      storage,
    ...
    const readback = buildParticipationDistributablePublicSeederReadback({
      proof,
      proofPath: output,
      readAt: argValue(argv, '--read-at') ?? new Date().toISOString()
    })
    const validationIssues = validateParticipationDistributablePublicSeederProof(proof)
    ...
    if (proof.status !== 'public_seeder_live') return proof.status === 'public_seeder_blocked' ? 2 : 1
...
async function runDown(argv) {
  ...
  if (!pidFile || !output) {
    console.error('Usage: node examples/participation-distributable-public-seeder-lifecycle.js down --pid-file <pid> --output <down> [--proof <proof>]')
    return 2
  }
  ...
  const down = buildParticipationDistributablePublicSeederDown({
    proof,
    stopped,
    signal: 'SIGTERM',
...
async function main(argv = process.argv.slice(2)) {
  ...
  console.error('Usage: node examples/participation-distributable-public-seeder-lifecycle.js <up|down> ...')
  return 2
}
